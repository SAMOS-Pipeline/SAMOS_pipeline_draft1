...
class WavelengthCalibration:
    ...
    def __call__(self,
        ...
        self.sci_target_file = ccd.header['SRP_FNAM']
        ...
        if len(comp_list) == 0:
            ...
            for self.lamp in comp_list:
                try:
                    self.calibration_lamp = self.lamp.header['SRP_FNAM']
                ...
                if self.wsolution is not None:
                    ccd.header.set('SRP_WRMS', value=self.rms_error)
                    ccd.header.set('SRP_WPOI', value=self.n_points)
                    ccd.header.set('SRP_WREJ', value=self.n_rejections)

                    linear_x_axis, self.lamp.data = linearize_spectrum(
                        self.lamp.data,
                        wavelength_solution=self.wsolution)

                    self.lamp = self.wcs.write_SRP_wcs(ccd=self.lamp,
    ...
    def _automatic_wavelength_solution(self,
        ...
        reference_lamp_wav_axis, reference_lamp_ccd.data = \
            self.wcs.read_SRP_wcs(ccd=reference_lamp_ccd)
    ...
    def _save_science_data(self,
        ...
        save_file_name = self._save_wavelength_calibrated(
            ccd=ccd,
            original_filename=ccd.header['SRP_FNAM'],
